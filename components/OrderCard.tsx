...
function statusBadge(status: KdsOrderStatus) {
  const map: Record<string, string> = {
    ...
    preparing: "bg-amber-50 text-amber-900 dark:bg-amber-950 dark:text-amber-100",
    ready:     "bg-emerald-50 text-emerald-900 dark:bg-emerald-950 dark:text-emerald-100",
  ...
  return map[status] ?? "bg-zinc-100 text-zinc-800 dark:bg-zinc-900 dark:text-zinc-200";
}
...
function nextPrimaryAction(status: KdsOrderStatus) {
  switch (status) {
    case "pending":   return { label: "Confirm",      to: "confirmed" as const };
    case "confirmed": return { label: "Start Cooking", to: "preparing" as const };
    case "preparing": return { label: "Mark Ready",    to: "ready" as const };
    case "ready":     return { label: "Complete",      to: "completed" as const };
    default:          return null;
  }
}
...
export function OrderCard({ order }: { order: OrderWithItems }) {
  ...
  return (
    <div className={`rounded-2xl border-2 bg-white shadow-sm dark:bg-zinc-950 transition-colors ${urgencyBorder(elapsed)}`}>
      {/* Header */}
      <div className={`rounded-t-2xl border-b border-zinc-100 dark:border-zinc-800 px-4 py-3 ${urgencyHeader(elapsed)}`}>
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1.5">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="text-lg font-bold tracking-tight">#{order.orderNumber}</h2>
              <span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold capitalize ${statusBadge(order.status)}`}>
                {order.status}
              </span>
              {elapsed >= 20 && (
          ...
          {/* Elapsed timer */}
          <div className={`rounded-xl border px-3 py-1.5 text-center shrink-0 ${
            elapsed >= 20
            ...
            <div className="text-[10px] uppercase tracking-wide text-zinc-500">Wait</div>
            <div className={`text-lg font-bold tabular-nums leading-none ${
              elapsed >= 20 ? "text-rose-700 dark:text-rose-300" :
              elapsed >= 10 ? "text-amber-700 dark:text-amber-300" : ""
            }`}>{elapsed}m</div>
          </div>
        </div>
      </div>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="text-right shrink-0">
                ...
                {item.status ? (
                  <p className="text-xs capitalize text-zinc-400">{item.status}</p>
                ) : null}
              </div>
            </div>
            ...
            {!!item.modifiers?.length && (
              <div className="mt-2 rounded-lg bg-white p-2 dark:bg-zinc-950">
                <p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-zinc-400">Modifiers</p>
                <div className="space-y-0.5">
                  {item.modifiers.map((mod) => (
                    <div key={mod.id} className="flex items-center justify-between text-xs">
                      ...
                      <span className="text-zinc-400">+Rs. {mod.priceDeltaSnapshot}</span>
                    </div>
                  ))}
                </div>
              </div>
